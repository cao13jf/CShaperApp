...
def run_shape_analysis(config, embryo_name):
    global max_time
    global cell_tree
    ...
    if not os.path.isdir(os.path.join(config['label_dict'], 'TemCellGraph')):
        os.makedirs(os.path.join(config['label_dict'], 'TemCellGraph'))
    ...
    for _ in tqdm(mpPool.imap_unordered(cell_graph_network, configs), total=len(configs),
    ...
    for itime in tqdm(range(1, max_time + 1), desc='assembling {} result'.format(embryo_name)):
        file_name = os.path.join(os.path.join(config['label_dict'], 'TemCellGraph'), config['embryo_name'],
    ...
    if config['delete_tem_file']:  # If need to delete temporary files.
        shutil.rmtree(os.path.join(config['label_dict'], 'TemCellGraph'))
...
def cell_graph_network(config):
    ...
    seg_file = os.path.join(config['seg_folder'], config['embryo_name'],
                            config['embryo_name'] + "_" + str(time_point).zfill(3) + '_segCell.nii.gz')
    nucleus_loc_file = os.path.join(os.path.join(config['label_dict'], 'TemCellGraph'), config['embryo_name'],
    ...
    for label in all_labels:
        ...
        point_graph.add_node(cell_name,
                             pos=nucleus_loc[nucleus_loc.nucleus_name == cell_name].iloc[:, 2:5].values[
                                 0].tolist())
...
def unify_label_seg_and_nuclues(file_lock, time_point, seg_file, config):
    ...
    nucleus_loc_to_save = pd.DataFrame.from_dict(
        {'nucleus_label': nucleus_number, 'nucleus_name': nucleus_names,
    ...
    save_name_fast_read = os.path.join(os.path.join(config['label_dict'], 'TemCellGraph'), config['embryo_name'],
                                       config['embryo_name'] + "_" + str(time_point).zfill(
                                           3) + '_nucLoc' + '.txt')
    ##################################################
    #  unify the segmentation label
    unify_seg = np.zeros_like(seg)  # TODO: update cell mother daughter's label
    changed_flag = np.zeros_like(
        seg)  # to label whether a cell has been updated with labels in the nucleus stack.
        ...
        if raw_label != 0:
            if not update_flag:
                ...
                nucleus_loc_to_save.loc[
                    nucleus_loc_to_save.nucleus_label == target_label, "surface"] = surface_area * (
                            config["res"] ** 2)
            else:
                ...
                if another_mother_name == mother_name:
                    ...
                    if mother_loc is not None:
                        ...
                        nucleus_loc_to_save.loc[
                            nucleus_loc_to_save.nucleus_label == mother_label, "surface"] = surface_area * (
                                    config["res"] ** 2)
                        ...
                        update_time_tree(config['embryo_name'], name_dict[target_label], time_point, file_lock,
                                         add=False)
                else:
                    # nucleus is occupied by strangers
                    nucleus_loc_to_save.loc[
                        nucleus_loc_to_save.nucleus_label == target_label, "note"] = "lost_inner2"
                    update_time_tree(config['embryo_name'], name_dict[target_label], time_point, file_lock,
                                     add=False)
        else:
    ...
    raw_labels = list(
        seg[nucleus_location_zoom[:, 0], nucleus_location_zoom[:, 1], nucleus_location_zoom[:, 2]])
    ...
    for repeat_label in repeat_labels:
        ...
        if name_dict[parent_label] not in cell_names:
            ...
            cell_locations.append(
                list(nucleus_location_zoom[repeat_label[0]]))  # TODO: change this when plot in 3D
...
class MainForm(QMainWindow, Ui_MainWindow):
    ...
    def chooseRawFolder_Pre(self):
        ...
        try:
            ...
            self.CB_embryoNames.addItems(os.listdir(dirName))
    ...
    def chooseStackFolder_Pre(self):
        ...
        try:
            ...
            self.CB_embryoNames_Seg.addItems(os.listdir(dirName))
            self.CB_embryoNames_Ana.clear()
            self.CB_embryoNames_Ana.addItems(os.listdir(dirName))
        except Exception as e:
            QMessageBox.warning(self, 'Warning!', 'Please Choose Right Folder!')
    ...
    def runPreprocess(self):
        ...
        try:
            ...
            QMessageBox.warning(self, '22222222222222', 'Please Choose Right Folder!22222222')
        ...
        if not config_test['only_post_process']:
            ...
            for i in tqdm(range(0, data_number), desc='Segmenting Data:'):
                ...
                if (config_test.get('save_binary_seg', False)):
                    ...
                    save_array_as_nifty_volume((pred).astype(np.int16),
                                               os.path.join("./ResultTem", "PredictedMap", emrbyo_name, img_names))
                else:
                    sess.close()
    ...
    def runAnalysis(self):
        ...
        para_config["label_dict"] = self.LE_shapeUtil_Ana.text()
        ...
        for embryo_name in embryo_names:
            ...
            para_config["acetree_file"] = os.path.join(para_config["stack_folder"], para_config['embryo_name'],
                                                       "CD" + para_config['embryo_name'] + ".csv")
